...
";
    ...
    {
        $this->assertGlobalVariables($event, $globals, true);
    }
    ...
    private function assertGlobalVariables(array $event, array $expectedGlobalVariables, bool $resolvePrefix = false): void
    {
        $handler = __DIR__ . '/PhpFpm/request.php';
        $prefixResolver = null;
        if ($resolvePrefix === true) {
            $prefixResolver = function (string $uri, array $event, FastCgiRequest $request) use ($handler) {
                $headers = array_change_key_case($event['headers'] ?? []);
                if (! isset($headers['forwarded']) && isset($event['requestContext']['path'])) {
                    $requestUri = $event['requestContext']['path'];
                    $request->setCustomVar('SCRIPT_NAME', rtrim(str_replace($uri, '/', $requestUri), '/') . '/' . basename($handler));
        ...
        $this->startFpm($handler, $prefixResolver);
    ...
    private function startFpm(string $handler, ?callable $prefixResolver = null): void
    {
        if ($this->fpm) {
            $this->fpm->stop();
        }
        $this->fpm = new PhpFpm($handler, __DIR__ . '/PhpFpm/php-fpm.conf', $prefixResolver);
        $this->fpm->start();
    }
}
